...
const GenreCard = ({ title, id, active, img_path }: GenreCardProps) => {
    ...
    const handleDelete = async () => {
        const res = await axios.delete(`/admin/genres/delete/${id}`, {
            headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.put('/admin/genres/update/' + id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
            ...
            if(res.status === 200) {
                ...
                closeEdit()
                toast.success('Genre mis à jour avec succès!')
                update({...session})
        }
        } catch (error: any) {
            ...
            <Modal isOpen={editIsOpen} onClose={closeEdit}>
                <form className="w-full max-w-lg rounded-lg bg-dark-800 border border-dark-700" onSubmit={handleSubmit}>
                    <div className="flex justify-between p-4 border-b border-dark-700">
                        <h2 className="text-white font-bold text-lg pt-1">Modifier le genre</h2>
                    ...
                    <div className="p-4 flex flex-col space-y-4 border-b border-dark-700">
                        ...
                        <div className="flex justify-between items-center space-x-4">
                            ...
                            <select 
                                ...
                                id="status"
                                value={genre?.status}
                                onChange={(e) => setGenre({...genre, status: e.target.value === "true"})}
                            >
