...
const CastingCard = ({ name, photoUrl, id, handleSelect, selected, entity }: { entity?: string, name: string, photoUrl: string, id: string, selected: boolean, handleSelect: (value: string) => void }) => {
    ...
    const handleDelete = async () => {
        try {
            const res = await axios.delete(`/admin/${entity}/delete/${id}`, {
                headers: {
    ...
    return (
        <div className="rounded-xl overflow-hidden bg-dark-800 shadow-secondary border border-dark-700">
            <div className="bg-black h-80 w-full relative">
                {photoUrl && (<Image className="object-cover object-top" src={photoUrl} alt={name} fill/>)}
            ...
            <div className="p-3">
                <div className="flex justify-between items-center">
                    <div className="flex space-x-3">
                        <Popup
                            trigger={open => (
                                <Link href={"/profiles/" + (entity === 'acteurs' ? 'actors' : 'directors') + "/" + id} className="h-7 w-7 bg-secondary-900 rounded-full flex items-center justify-center">
