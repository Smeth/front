...
const CategoryPodCard = ({ title, id, active, img_path }: GenreCardProps) => {
    ...
    const handleDelete = async () => {
        const res = await axios.delete(`/admin/catpod/delete/${id}`, {
            headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.put('/admin/catpod/update/' + id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
...
export default CategoryPodCard