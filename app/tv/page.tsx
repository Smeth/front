...
export default function Movies() {
    ...
    const [chanels, setChanels] = useState<any[]>([])
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/tv/all/?page='+currentPage+'&size='+pageSize+'', {
                    headers: {
                        Authorization: `Bearer ${session?.token}`
                    }
                })
                if(response.status === 200) {
                    setIsLoading(false)
                    setChanels(response.data.Data.content)
            ...
            <div className="w-full bg-dark-800 rounded p-5">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between">
                    ...
                    <div className="">
                        <Popup
                            trigger={open => (
                                <Link href="/tv/add" className="flex items-center space-x-2 bg-secondary-900 py-1.5 px-2 rounded">
                                    <PlusIcon classes="h-3.5 text-white" />
                                    <span className="text-white text-sm font-semibold">Ajouter une chaine live</span>
                                </Link>
                            )}
                            position="top center"
                            on={['hover', 'focus']}
                        >
                            <Tooltip title="Ajouter une chaine live" />
                        </Popup>
                    </div>
                </div>
                ...
                <div className={"grid gap-6 my-6 " + (!isLoading && chanels.length === 0 ? " grid-cols-1" : "grid-cols-4 3xl:grid-cols-5 4xl:grid-cols-6")}>
                    {!isLoading && chanels.length > 0 && chanels.map((live: any, i: number) => (
                        <TVCard 
                            key={i}
                            title={live.name}
                            id={live.idLiveTv}
                            active={live.status}
                            selected={false}
                            image={live.tvLogo_path}
                        />
                    ))}
                    {!isLoading && chanels.length === 0 && (
                        <div className="w-full bg-black py-6">
                            <p className="text-white text-center font-medium p-6">
                                {"Aucune chaine tv n'a encore été ajoutée, "}
                                <Link href="/tv/add" className="font-semibold underline text-blue-500">ajoutez-en !</Link>
                            </p>
                        </div>
                    )}
