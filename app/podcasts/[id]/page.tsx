...
export default function AddPodcastPage({ params }: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/podcasts/' + params.id, {
                    headers: {
                        Authorization: `Bearer ${session?.token}`
                    }
                })
                if(response.status === 200) {
                    ...
                    setPodcast({
                        ...
                        idCollection: [collectionOptions.find((l: any) => l.value === response.data.Data.idCollection)],
                    })
                }
                else {
                    toast.error("Une erreur est survenue. Veuillez rafraichir la page.")
    ...
    }, [session, params.id, router, languesOptions, collectionOptions, categoriesOptions])
    ...
    console.log('languesOptions', languesOptions)
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.put('/admin/podcasts/update/' + params.id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
                                    ...
                                    <div className="flex justify-between items-center space-x-4">
                                        ...
                                        <div className="w-full md:w-2/3">
                                            <SelectSearch
                                                ...
                                                isMulti={false}
                                                defaultValue={podcast?.langue || null}
                                                onChange={handleSelectSearchChange}
                                    ...
                                    <div className="flex justify-between items-center space-x-4">
                                        ...
                                        <div className="w-full md:w-2/3">
                                            <SelectSearch
                                                ...
                                                isMulti={true}
                                                defaultValue={podcast?.categories || null}
                                                onChange={handleSelectSearchChange}
