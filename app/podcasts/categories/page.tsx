...
import CategoryPodCard from "@components/cards/CategoryPodCard"
...
export default function Categories() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/catpod/all/?page=' + currentPage + '&size=' + pageSize + '&sort=name', {
                    headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.post('/admin/catpod/create', data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
        ...
        <div className="w-full">
            <div className="w-full bg-dark-800 rounded p-6">
                ...
                <div className={"grid gap-6 my-6 " + (!isLoading && categories.length === 0 ? " grid-cols-1" : "grid-cols-4 3xl:grid-cols-5 4xl:grid-cols-6")}>
                    ...
                    {isLoading && <PlaceholderShortCards nbCards={12} />}
                </div>
