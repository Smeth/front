...
export default function SeriePage({params}: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/series/' + params.id, {
                    headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.put('/admin/series/update/' + params.id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
            ...
            if(res.status === 200) {
                toast.success('Série mis à jour avec succès!')
