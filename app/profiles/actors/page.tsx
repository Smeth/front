...
export default function ActorsPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/acteurs/all/?page='+currentPage+'&size='+pageSize+'&sort=fistName', {
                    headers: {
                ...
                if(response.status === 200) {
                    setIsLoading(false)
                    setActors(response.data.Data.content)
                ...
                <div className="mt-3 rounded p-2.5 bg-dark-500 border text-grey-100 border-dark-600 flex flex-col md:flex-row justify-between items-center">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="">
                            <Popup
                                trigger={open => (
                                    <Link href="/profiles/actors/add" className="flex items-center space-x-2 bg-secondary-900 py-1.5 px-2 rounded">
                                        <PlusIcon classes="h-3.5 text-white" />
                                        <span className="text-white text-sm font-semibold">Ajouter un acteur</span>
                                    </Link>
                                )}
                                position="top center"
                                on={['hover', 'focus']}
                            >
                                <Tooltip title="Ajouter un acteur" />
                            </Popup>
                        </div>
                    </div>
                </div>
                <div className={"grid gap-6 my-6 " + (!isLoading && actors.length === 0 ? " grid-cols-1" : "grid-cols-5")}>
                    {!isLoading && actors.length > 0 && actors.map((actor: any, i: number) => (
                        <CastingCard 
                            key={i} 
                    ...
                    {!isLoading && actors.length === 0 && (
                        <div className="w-full bg-black py-6">
                            <p className="text-white text-center font-medium p-6">
                                {"Aucun acteur n'a encore été ajoutée, "}
                                <Link href="/profiles/actors/add" className="font-semibold underline text-blue-500">ajoutez-en !</Link>
                            </p>
                        </div>
                    )}
