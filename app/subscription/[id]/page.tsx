...
export default function SubscriptionPage({ params }: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/subscriptionTypes/' + params.id, {
                    headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const data = {
            ...
            deviceLimit: parseInt(deviceLimit.value),
            nbMonth: parseInt(nbMonth.value),
            status: JSON.parse(status.value),
            ads: JSON.parse(ads.value)
        }

        try {
            const res = await axios.put('/admin/subscriptionTypes/update/' + params.id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
            ...
            if(res.status === 200) {
                toast.success("Plan d'abonnement modifié avec succès!")
