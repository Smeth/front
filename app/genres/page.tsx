...
import { ImportIcon, PlusIcon, TimesIcon } from "@icons/MyTVIcons"
...
export default function Genres() {
    ...
    const handleImportGenres = async () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/genres/all/?page='+currentPage+'&size='+pageSize+'&sort=name', {
                    headers: {
                        Authorization: `Bearer ${session?.token}`
                    }
                })
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.post('/admin/genres/create', data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
    ...
    return (
        <div className="w-full  ">
            <div className="w-full bg-dark-800 rounded p-6">
                        ...
                        <Popup
                            trigger={open => (
                                <button className="flex items-center space-x-2 bg-secondary-900 py-1.5 px-2 rounded" onClick={() => setOpen(true)}>
                                    ...
                                    <span className="text-white text-sm font-semibold">Ajouter un genre</span>
                                </button>
                            )}
                            position="top center"
                            on={['hover', 'focus']}
                        >
                            <Tooltip title="Ajouter un genre" />
                        </Popup>
                        <Popup
                            trigger={open => (
                                <button onClick={handleImportGenres} className="flex items-center space-x-2 bg-tertiary-800 py-1.5 px-2 rounded">
                                    <ImportIcon classes="h-5 text-white" />
                                    <span className="text-white text-sm font-semibold">{"Importer depuis TheMovieDb"}</span>
                                </button>
                            )}
                            position="top center"
                            on={['hover', 'focus']}
                        >
                            <Tooltip title={"Importer depuis TheMovieDb"} />
                        </Popup>
                    </div>
                    ...
                    <Modal isOpen={open} onClose={closeModal}>
                        ...
                        >
                            <div className="flex justify-between p-4 border-b border-dark-700">
                                <h2 className="text-white font-bold text-lg pt-1">Ajouter un genre</h2>
                            ...
                            <div className="p-4 flex flex-col space-y-4 border-b border-dark-700">
                                ...
                                <div className="flex justify-between items-center space-x-4">
                                    <label htmlFor="name" className="text-white text-sm font-medium">Titre du genre</label>
                ...
                <div className={"grid gap-6 my-6 " + (!isLoading && genres.length === 0 ? " grid-cols-1" : "grid-cols-4 3xl:grid-cols-5 4xl:grid-cols-6")}>
                    {!isLoading && genres.length > 0 && genres.map((genre: any, i: number) => (
                       <GenreCard
                            key={i}
                            title={genre.name}
                            active={genre.status}
                    ...
                    {!isLoading && genres.length === 0 && (
                        <div className="w-full bg-black py-6">
                            <p className="text-white text-center font-medium p-6">
                                {"Aucun genre n'a encore été ajouté, "}
