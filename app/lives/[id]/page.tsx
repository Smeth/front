...
export default function LivePage({ params }: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/admin/lives/' + params.id, {
                    headers: {
                        Authorization: `Bearer ${session?.token}`
                    }
                })
                if(response.status === 200) {
                    ...
                    setLive({
                        ...response.data.Data,
                        idCats: response.data.Data.idCats.map((id: number) => categoriesOptions.find((cat: any) => cat.value === id))
                    })
                }
                else {
                    toast.error("Une erreur est survenue. Veuillez rafraichir la page.")
        ...
        session?.token ? fetchData() : null
    }, [session, params.id, router, categoriesOptions])
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await axios.put('/admin/lives/update/' + params.id, data, {
                headers: {
                    Authorization: `Bearer ${session?.token}`
                }
            })
                            ...
                            <div className="mt-8 flex flex-col gap-4">
                                ...
                                <div className="flex justify-between items-center space-x-4">
                                    ...
                                    <select 
                                        ...
                                        name="accessFree" 
                                        id="accessFree"
                                        value={live?.accessFree}
                                        onChange={(e: any) => setLive({...live, accessFree: e.target.value === "true"})}
                                    >
